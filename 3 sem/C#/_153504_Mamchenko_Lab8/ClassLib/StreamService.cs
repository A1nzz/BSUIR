...
namespace ClassLib
{
    public class StreamService<T>
    {
        ...
        {
            progress.Report($"Начало записи в поток {Thread.CurrentThread.ManagedThreadId}");
           ...
           // await Process();
            ...
            progress.Report($"\nКонец записи в поток {Thread.CurrentThread.ManagedThreadId}");
        }

        public async Task CopyFromStreamAsync(Stream stream, string fileName, IProgress<string> progress)
        {
            progress.Report($"Начало считывания из потока {Thread.CurrentThread.ManagedThreadId}");
            ...
            progress.Report($"\nКонец считывания из потока {Thread.CurrentThread.ManagedThreadId}");
        }
        ...
        //        Console.Write($"\rПоток {Thread.CurrentThread.ManagedThreadId} {m}");
        //    });
